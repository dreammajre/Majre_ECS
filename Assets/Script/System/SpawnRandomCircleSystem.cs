...
#pragma warning restore 649
        ...
        protected override void OnUpdate()
        {
            while (m_Group.Length != 0)
            {
                ...
                if (spawner.spawnLocak)
                {
                    GeneratePoints.RandomPointsOnCircle(new float3(), spawner.readius, ref positions);
                ...
                {
                    GeneratePoints.RandomPointsOnCircle(center, spawner.readius, ref positions);
                    for (int i = 0; i < spawner.count; i++)
                    {
                        var position = new Position
                        {
